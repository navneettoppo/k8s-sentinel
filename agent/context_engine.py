...
class ContextEngine:
    ...
    async def get_cluster_stats(self) -> Dict[str, Any]:
        ...
        stats = {
            "nodes": len([n for n in self.topology_graph.values() if n.resource_type == "node"]),
            "pods": len([n for n in self.topology_graph.values() if n.resource_type == "pod"]),
            "services": len([n for n in self.topology_graph.values() if n.resource_type == "service"]),
            "deployments": len([n for n in self.topology_graph.values() if n.resource_type == "deployment"]),
            "pvcs": len([n for n in self.topology_graph.values() if "pvc" in n.resource_type]),
            "healthy": 0,
            "total_alerts": len(self.incidents)
        }
        # Count healthy resources
        for node in self.topology_graph.values():
            status = node.properties.get("status", "").lower()
            if status in ["ready", "running", "bound", "active"]:
                stats["healthy"] += 1
        return stats

    async def get_topology(self) -> Dict[str, Any]:
        """Get resource topology graph for dashboard."""
        nodes = []
        edges = []
        for res_id, node in self.topology_graph.items():
            nodes.append({
                "id": res_id,
                "type": node.resource_type,
                "name": node.name,
                "status": node.properties.get("status", "Unknown"),
                "parents": list(node.relationships)[:5],
                "children": []
            })
